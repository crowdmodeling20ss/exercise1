...
class Dijkstra:
    ...
    def calculate_cost_map(self):
        ...
        for t in self.grid_map.get_target_positions():  # djikstra calculates cost according to all targets
            ...
            while self.__queue.empty() == False:
                ...
                if (self.__get_cost(current_position) != 0 and self.__get_cost(
                        current_position) > new_cost):  # if there exists a cost but new cost is lower.
    ...
    def __add_neighbour_to_queue_target(self,
                                        position):  # since targets do not have parents this function is just for the inital target cell
        for d in DIRECTIONS:  # check all directions
            neighbour_x = position[0] + d[0]
            neighbour_y = position[1] + d[1]
            if 0 <= neighbour_x < len(self.data) and 0 <= neighbour_y < len(
                    self.data[0]):  # check if outside the bounds
                n_state = self.grid_map.get_state([neighbour_x, neighbour_y])  # get the state of neighbour
                if (n_state != S_OBSTACLE) and (
                        n_state != S_TARGET):  # check if available cell (not obstacle or not target)
    ...
    def __add_neighbour_to_queue(self, position, parent):
        ...
        for d in DIRECTIONS:
            neighbour_x = position[0] + d[0]
            neighbour_y = position[1] + d[1]
            if 0 <= neighbour_x and neighbour_x < len(self.data) and 0 <= neighbour_y and neighbour_y < len(
                    self.data[0]):  # check if outside the bounds
                n_state = self.grid_map.get_state([neighbour_x, neighbour_y])  # get the state of neighbour
