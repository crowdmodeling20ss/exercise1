...
class CellularModel:
    ...
    def __init__(self, grid_map, is_pedestrian_exit=True, speeds=None, speed_per_pedestrian=False, obstacle_avoidance_enabled=True):
        self.pedestrians = []
    ...
    def import_pedestrians_from_map(self):
        ...
        for p_id, corners in enumerate(self.grid_map.corners):
            ...
            self.pedestrians.append(Pedestrian(p_id, self, self.grid_map, center, self.get_pedestrian_speed(p_id),
                                               self.grid_map.corners[p_id], r_max, self.is_pedestrian_exit, self.grid_map.is_dijkstra_enabled, self.obstacle_avoidance_enabled))
